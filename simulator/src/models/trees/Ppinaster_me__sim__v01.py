...
class PinusPinasterSIM(TreeModel):
    ...
    def initialize(self, plot: Plot):
        ...
        print('#----------------------------------------------------------------------------#')
        print('   Pinus pinaster mesogeensis model (Sistema Ibérico Meridional) is running   ')
        print('#----------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            plot.add_value('SI', math.exp(4.016 + (math.log(plot.dominant_h) - 4.016) * math.pow(
                                 t / plot.age, -0.5031)))  # Site Index (m) calculation
            ...
            for tree in plot_trees:  # for each tree...
                ...
                tree.set_value('normal_circumference', math.pi * tree.dbh)  # normal (at 1.30m) circumference (cm) calculation
    ...
    def survives(self, time: int, plot: Plot, tree: Tree):
        ...
        """
        ba_survives = 1 - (1 / (1 + math.exp(
            2.0968 + (4.7358 * tree.dbh / plot.qm_dbh) - 0.0012 * plot.si * plot.basal_area)))
        if ba_survives > 0:
            return ba_survives
        return 0.0
    ...
    def new_tree_distribution(self, time: int, plot: Plot, area: float):
        ...
        cd_3 = [22.5, sys.float_info.max, 0.5828 * area]
    ...
    def crown(self, tree: Tree, plot: Plot, func):
        ...
        tree.add_value('lcw', (1 / 10.0) * (0.1826 * tree.dbh * 10) * math.pow(tree.cr, (
                    0.1594 + 0.0014 * (tree.height - tree.hcb) * 10)))  # maximum crown-width (m) calculation
