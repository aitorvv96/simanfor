...
# Pinus halepensis model (Cataluña, Spain), version 01
# Written by iuFOR
...
class PinusHalepensisCataluña(TreeModel):


    def __init__(self, configuration=None):
        super().__init__(name="Pinus halepensis - Cataluña", version=1)
    ...
    def initialize(self, plot: Plot):
        ...
        print('#----------------------------------------------------------------------------#')
        print('                 Pinus halepensis model (Cataluña) is running                 ')
        print('#----------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if tree.height == 0:  # if the tree hasn't height (m) value, it is calculated
                    tree.add_value('height', 1.3 + (plot.mean_h - 1.3) * math.exp(0.3532 * (1 - plot.qm_dbh/tree.dbh)) * math.exp(
                        0.0315 * (tree.dbh/plot.qm_dbh - 1/tree.dbh)))
    ...
    def survives(self, time: int, plot: Plot, tree: Tree):
        ...
        # ELE =
        # SLO =
        # psurvive10 = 1 / (1 + math.exp(
        #    - (beta0 + beta1 * tree.bal / math.log(tree.dbh + 1)) + beta2 * tree.dbh / GI + beta3 * math.log(
        #        ELE) + beta4 * (SLO)))  # 10 years period
        # psurvive5 = pow(psurvive10, 0.5)  # 5 years period change
        # return psurvive5

        return 1  # by the moment the equation is silenced because of the variables are not on initial inventory
    ...
    def grow(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        beta5 = 0.0410
        beta6 = 0.5662
        dbhg10 = math.exp(beta1 + beta2 / old_tree.dbh + beta3 * old_tree.dbh / GI + beta4 * old_tree.bal / (
            math.log(old_tree.dbh + 1)) + beta5 * BALthin + beta6 * math.log(GI))
    ...
    def merch_classes(self, tree: Tree):
        ...
        usage, merch_list = TreeModel.merch_calculation(tree, class_conditions, PinusHalepensisCataluña)
